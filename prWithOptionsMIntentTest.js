let paymentRequestTimeout, paymentRequestPhonepe, paymentRequestGPay, timeOutCounter= 0;
...
function onProceedSelectedAppHasEnrolledInstrument(evt) {
  ...
  function abortPhonepePaymentRequest(cbFunction) {
    paymentRequestPhonepe.abort().then(cbFunction).catch(cbFunction);
  }
  
  function abortGPayPaymentRequest(cbFunction) {
    paymentRequestGPay.abort().then(cbFunction).catch(cbFunction);
  }
  
  function callBackForPhonepeHasEnrolledInstrument() {
  ...
  function callBackForGPayHasEnrolledInstrument() {
